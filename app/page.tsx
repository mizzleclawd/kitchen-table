...
function DetailView({ recipeId, onBack }: { recipeId: string; onBack: () => void }) {
  ...
  if (data === undefined) return <main className="min-h-screen bg-[#fbf5ea] p-8">Loading family recipe…</main>;
  if (data === null) return <main className="min-h-screen bg-[#fbf5ea] p-8"><button onClick={onBack} className="back text-[#765f4d]"><ArrowLeft size={18}/>The family book</button><p className="mt-8 text-[#685545]">That recipe is no longer in the family book.</p></main>;
